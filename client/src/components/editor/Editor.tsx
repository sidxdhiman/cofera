...
function Editor() {
    ...
    const { activeFile, setActiveFile, updateFileContent, fileStructure } = useFileSystem()
    ...
    const [showHighlightWarning, setShowHighlightWarning] = useState<boolean>(false)
    ...
    useEffect(() => {
        const handleSave = (e: KeyboardEvent) => {
            if ((e.ctrlKey || e.metaKey) && e.key === "s") {
                ...
                toast.success("File saved locally", { id: "save-file" })
            }
        }
        window.addEventListener("keydown", handleSave)
        return () => window.removeEventListener("keydown", handleSave)
    }, [currentUser.roomId, fileStructure, socket])
